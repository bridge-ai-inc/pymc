 ...
 *                  myname_F.bin    = fluence rate output F[i] [W/mm^2 per W delivered]
 *
 ...
 *          1. Fluence rate F [W/mm^2 per W delivered]
 *          2. Deposition rate A [W/mm^3 per W delivered].
 *
 *  Log:
...
int main(int argc, const char * argv[]) {
	...
	double	s;              /* step sizes. s = -log(RND)/mus [mm] */
	...
	/* other variables */
	double	mua;            /* absorption coefficient [mm^-1] */
	double	mus;            /* scattering coefficient [mm^-1] */
	double	g;              /* anisotropy [-] */
	...
	fgets(buf, 32, fid);
		...
		fgets(buf, 32,fid);
		sscanf(buf, "%f", &dx);	 // size of bins [mm]
		fgets(buf, 32,fid);
		sscanf(buf, "%f", &dy);	 // size of bins [mm] 
		fgets(buf, 32,fid);
		sscanf(buf, "%f", &dz);	 // size of bins [mm] 
		...
		for (i=1; i<=Nt; i++) {
			fgets(buf, 32, fid);
			sscanf(buf, "%f", &muav[i]);	// absorption coeff [mm^-1]
			fgets(buf, 32, fid);
			sscanf(buf, "%f", &musv[i]);	// scattering coeff [mm^-1]
			fgets(buf, 32, fid);
    ...
    for (i=1; i<=Nt; i++) {
        printf("muav[%ld] = %0.4f [mm^-1]\n",i,muav[i]);
        printf("musv[%ld] = %0.4f [mm^-1]\n",i,musv[i]);
        printf("  gv[%ld] = %0.4f [--]\n\n",i,gv[i]);
    }
	...
	F  = (float *)malloc(NN*sizeof(float));	/* relative fluence rate [W/mm^2/W.delivered] */
	// NOT READY: R  = (float *)malloc(NN*sizeof(float));	/* escaping flux [W/mm^2/W.delivered] */
    ...
    printf("Cartilage thickness: %0.1f mm\n",Nx*dx);
	...
	do {
		...
		do {
			...
			do{  // while sleft>0   
				s     = sleft/mus;				/* Step size [mm].*/
				tempx = x + s*ux;				/* Update positions. [mm] */
				tempy = y + s*uy;	
				tempz = z + s*uz;
