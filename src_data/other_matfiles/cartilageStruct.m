...
%
%   To use, 
%       1. Prepare makeCartilageList.m so it contains desired tissue types.
%       2. Specify the USER CHOICES.
%       2. Run this program, cartilageStruct.m
%
%   Note: mcxyz.c can use optical properties in cm^-1 or mm^-1 or m^-1,
%         if the bin size (binsize) is specified in cm or mm or m, resp.
%
%   Isaac Afara adopted from Steven L. Jacques
...
myname      = 'cartilage';% name for files: myname_T.bin, myname_H.mci  
time_min    = 10;      	% time duration of the simulation [min] <----- run time -----
nm          = 1080;   	% desired wavelength of simulation
Nbins       = 200;    	% # of bins in each dimension of cube 
binsize     = 0.005; 	% size of each bin [mm]
...
radius      = 0.300;   % 1/e radius of beam at tissue surface
waist       = 0.300;  	% 1/e radius of beam at focus
...
%   for each voxel in T.
% Tissue thickness [mm] >> Nbins * binsize = 200 * 0.005 = 1 mm

T = double(zeros(Ny,Nx,Nz)); 

T = T + 4;      % fill background with deep zone

zsurf = 0.0100;  % position of air/articular surface interface
...
for iz=1:Nz % for every depth z(iz)
    ...
    % superficial zone (150 um thick)
    if iz>round(zsurf/dz) & iz<=round((zsurf+0.150)/dz)
        T(:,:,iz) = 2; 
    end
    
    % mddile zone (250 um thick)
    if iz>round((zsurf+0.150)/dz) & iz<=round((zsurf+0.400)/dz)
        T(:,:,iz) = 3; 
    end

    % deep zone (600 um thick) will be the remaining
