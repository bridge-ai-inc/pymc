function status = cartilage3Dstruct_dev(nm, tissueName)

% MAIN OUTPUT: status = status of computation
% outputs: myname_T.bin = a tissue structure file
%          myname_H.mci = the input file for use by mcxyz.c.
%   Isaac Afara adopted from Steven L. Jacques
%  CHANGES: number of beam; Nbins
...
time_min    = 20;      	% time duration of the simulation [min] <----- run time -----
...
waist       = 0.2500;  	% 1/e radius of beam at focus
...
% Create T(y,x,z) by specifying tissue type (integer)for each voxel in T.

T = double(zeros(Ny,Nx,Nz)); 
T = T + 4;        % fill background with deep zone
zsurf = 0.0025;   % position of air/articular surface
...
if SAVEON
    ...
    %commandwindow
    %disp(sprintf('--------create %s --------',tissueName))
...
disp('Cartilage structure created!')
